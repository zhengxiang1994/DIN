...
def train(
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        sys.stdout.flush()
        print('test_auc: %.4f ---- test_loss: %.4f ---- test_accuracy: %.4f' % eval(sess, test_data, model,
                                                                                    best_model_path))
        sys.stdout.flush()
        ...
        for itr in range(3):
            ...
            for src, tgt in train_data:
                ...
                if (iter % test_iter) == 0:
                    ...
                    iter, loss_sum / test_iter, accuracy_sum / test_iter / test_iter))
                    print('test_auc: %.4f ----test_loss: %.4f ---- test_accuracy: %.4f' % eval(
...
def test(
    ...
    model_path = tf.train.latest_checkpoint('./dnn_best_model/')
    gpu_options = tf.GPUOptions(allow_growth=True)
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        print('test_auc: %.4f ----test_loss: %.4f ---- test_accuracy: %.4f' % eval(sess,
                                                                                   test_data,
                                                                                   model,
                                                                                   model_path))
